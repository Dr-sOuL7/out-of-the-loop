...
class GameEngine:
    ...
    async def record_private_answer(
        ...
        if (
            ...
            if game.current_round.guess_options:
                await self._dm(context, user.id, texts.guess_use_buttons())
            else:
    ...
    async def _start_imposter_guess(
        ...
        decoys = await self.content.decoy_words(rnd.category, rnd.secret_word)
        if decoys:
            ...
            ok = await self._dm(
                ...
                reply_markup=self._guess_keyboard(rnd),
            )
        else:
            ...
            ok = await self._dm(
                context,
                rnd.imposter_id,
                texts.imposter_guess_prompt(cat_name, seconds),
            )
        await self._send(
            context,
            game.chat_id,
    ...
    def _guess_keyboard(self, rnd: Round) -> InlineKeyboardMarkup:
    ...
    async def cast_guess(
        ...
        """Handle the imposter tapping one of the guess-option buttons."""
        chat_id = self._player_index.get(user_id)
        game = self.games.get(chat_id) if chat_id is not None else None
        if game is None:
        ...
        async with self._lock(game.chat_id):
            ...
            if dm_message is not None:
                try:
                    ...
                    pass
            if not edited:
                await self._dm(context, user_id, outcome)
            await self._do_scoring(context, game)
